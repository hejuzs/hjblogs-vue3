                ...
                <div class="flex">
                    ...
                    <div class="grow" :class="[isExpand ? 'block' : 'hidden 2xl:block']">
                        ...
                        <div id="accordion-flush" data-accordion="collapse"
                            ...
                            data-inactive-classes="" class="last:pb-[170px]">
...
getWikiCatalogs(route.params.wikiId).then(res => {
    if (res.success) {
        ...
        updateCatalogExpansion(route.query.articleId)
    }
})
...
watch(route, (newRoute, oldRoute) => {
    ...
    // 更新目录展开状态
    updateCatalogExpansion(newRoute.query.articleId)
})

// 添加一个方法来更新目录展开状态（主要是为了解决，点击 上一篇文章 或 下一篇文章 时，如果跨了目录的话没有自动展开的问题）
function updateCatalogExpansion(articleId) {
    nextTick(() => {
        // 找到包含当前文章的目录项
        const targetCatalog = catalogs.value.find(catalog => 
            catalog.children && catalog.children.some(child => child.articleId == articleId)
        )
        ...
        if (targetCatalog) {
            ...
            // 展开目标目录项
            const targetButton = document.querySelector(`[data-accordion-target="#accordion-flush-body-${targetCatalog.id}"]`)
            if (targetButton) {
                // 等待一小段时间确保关闭操作完成
                setTimeout(() => {
                    targetButton.click() // 模拟点击展开
                }, 50)
            }
        }
    })
